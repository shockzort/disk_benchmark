...
class FioBenchmark(BenchmarkModule):
    """Base class for fio benchmark modules."""

    def __init__(self, name: str = "fio"):
        super().__init__(name)

    def is_available(self) -> bool:
        """Check if fio is available."""
        return check_command_exists(self.name)

    @abstractmethod
    def run(self, device_path: str, mount_point: str) -> BenchmarkResult:
        """Run the specific fio benchmark test."""
        pass
...
class FioWriteBenchmark(FioBenchmark):
    """fio benchmark for write testing."""

    def run(self, device_path: str, mount_point: str) -> BenchmarkResult:
        """Run fio write benchmark."""
        self.logger.info(f"Running fio write benchmark on {mount_point}")
        ...
        try:
            ...
            io_size = self.config.fio_write_io_size if self.config else "1G"
...
class FioRandRWBenchmark(FioBenchmark):
    """fio benchmark for random read-write testing."""

    def run(self, device_path: str, mount_point: str) -> BenchmarkResult:
        """Run fio random read-write benchmark."""
        self.logger.info(f"Running fio random read-write benchmark on {mount_point}")
        ...
        try:
            ...
            io_size = self.config.fio_randrw_io_size if self.config else "1G"
